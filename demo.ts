...
import { ResourceType, SubscriptionStatus, Subscription, AccessGrant } from "./types";
...
function initializeDemoData(): void {
  ...
  const clusterSubscription = subscriptionManager.registerSubscription(
    "user-alice",
    ResourceType.SSH_CLUSTER,
    100, // usage limit (e.g., 100 hours)
  ...
  const gpuSubscription = subscriptionManager.registerSubscription(
    "user-bob",
  ...
  const chatgptSubscription = subscriptionManager.registerSubscription(
    "user-alice",
    ResourceType.CHATGPT_API,
    1000000, // 1M tokens limit
 ...
 */
async function demonstrateResourceRental(): Promise<void> {
  console.log("\n🔍 Finding available SSH Cluster resources...\n");

  // Find available SSH cluster subscriptions with at least 50% capacity
  const availableClusters = subscriptionManager.findAvailableSubscriptions(
    ResourceType.SSH_CLUSTER,
    50 // minimum 50% capacity available
  );
  ...
  const duration = parseDuration("4h"); // 4 hours
  ...
  // Create access grant
  const grant = accessGrantManager.createGrant(
 ...
 */
function demonstrateRevocation(): void {
  console.log("\n" + "=".repeat(50));
  console.log("\n🚫 Demonstrating Access Revocation\n");

  // Create a temporary grant to revoke
  const tempGrant = accessGrantManager.createGrant(
    "temp-subscription",
    "user-dave",
    parseDuration("1h"),
    accessGrantManager.encryptCredentials({ test: "data" })
  );
...
function demonstrateValidation(): void {
  ...
  console.log(`   Email "${validEmail}": ${validateEmail(validEmail) ? "✅ Valid" : "❌ Invalid"}`);
  ...
  console.log(`   SSH key (valid format): ${validateSSHPublicKey(validSSHKey) ? "✅ Valid" : "❌ Invalid"}`);
  console.log(`   SSH key (invalid format): ${validateSSHPublicKey(invalidSSHKey) ? "✅ Valid" : "❌ Invalid"}`);
}
